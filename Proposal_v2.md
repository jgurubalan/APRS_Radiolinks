...
Throughput
To compare the behavior of:
UDP-based transmission (loss-tolerant, real-time style)
TCP-based transmission (reliable but potentially slower)
To evaluate the suitability of the AMPR network for:
Telemetry data
Weather station data
Similar sensor-based applications
To build a small experimental IP-based ham network platform that can later be extended to RF links.
...
4. Methodology
Node A will transmit telemetry packets containing:
Sequence number
Timestamp
...
Future IP-over-RF experimentation
8. Future Extensions
